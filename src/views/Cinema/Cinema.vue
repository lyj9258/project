<template>
  <div class="main">
    <div class="cinema-list">
      ...
      <div class="cinema-banner-wrap" v-for="(res,ret) in data" :key="ret">
        <ul class="cinema-list">
          <li class="cinema-banner-item">
            <div class="name">
              <span class="title">{{res.masterTitle}}</span>
              <span class="icon">影院券</span>
            </div>
            <div class="type">{{res.name}}</div>
          </li>
        </ul>
      </div>
      <div class="cinema-list-wrap">
        <ul class="cinema-list">
          <div
            ...
            :key="index"
          >
            <a href="#/cinema/1000/film" class="cinema-item-wrap">
...
import { cinemaListData ,bannersData} from "@/api/api";
export default {
  data() {
    return {
      ticketFlag:1,
  ...
  async mounted() {
    ...
    let ban = await bannersData();
    this.data = ban.data.data;
